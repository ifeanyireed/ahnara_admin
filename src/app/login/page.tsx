...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setTimeout(() => {
        setIsLoading(false);
        const role = email.toLowerCase().includes("super") ? "SUPER" : "ADMIN";
        login(role === "SUPER" ? "mock-token-super" : "mock-token-admin", {
          id: role === "SUPER" ? "mock-super-id" : "mock-admin-id",
          email: email,
          name: role === "SUPER" ? "Super Admin" : "Country Admin",
          role: role,
        });
        router.push(role === "SUPER" ? "/system" : "/country");
      }, 1000);
    } catch (err: any) {
      setIsLoading(false);
  ...
  const handleQuickLogin = (role: "ADMIN" | "SUPER") => {
    setIsLoading(true);
    setError(null);
    setTimeout(() => {
      setIsLoading(false);
      login(role === "SUPER" ? "mock-token-super" : "mock-token-admin", {
        id: role === "SUPER" ? "mock-super-id" : "mock-admin-id",
        email: role === "SUPER" ? "super@ahnara.com" : "country@ahnara.com",
        name: role === "SUPER" ? "Super Admin (System)" : "Country Admin (Portal)",
        role: role,
      });
      router.push(role === "SUPER" ? "/system" : "/country");
    }, 600);
      ...
      >
        <AhnaraCard variant="flat" className="bg-white/80 backdrop-blur-md border border-slate-200/80 p-8 shadow-2xl rounded-3xl flex flex-col gap-6">
          ...
          <div className="flex flex-col items-center text-center gap-2">
            ...
            <p className="text-xs text-slate-400 font-semibold">Access your Ahnara Admin workspaces</p>
          </div>
